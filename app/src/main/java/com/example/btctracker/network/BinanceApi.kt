...
object BinanceApi {
    ...
    suspend fun getBTC(key: String, secret: String): Double {
        return withContext(Dispatchers.IO) {
            ...
            val signature = mac.doFinal(query.toByteArray())
                               .joinToString("") { "%02x".format(it) }

            val url = "https://api.binance.com/api/v3/account?$query&signature=$signature"
            val jsonString = HttpClient.get(url, mapOf("X-MBX-APIKEY" to key))
